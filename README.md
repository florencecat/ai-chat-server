...
Разметка допустима только в `response`. Из `name`, `text`, `note`, `food`,
`red_flags` и `follow_up_questions` она снимается на сервере: клиент рисует
эти поля обычным текстом, и `приём **Пульмикорта**` пользователь увидел бы
вместе со звёздочками.
...
| `PROMPT_VERSION`       | `p2`                          | Входит в ключ кэша и в тело ответа                  |
