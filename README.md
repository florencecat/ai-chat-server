...
```jsonc
{
  ...
  "red_flags": ["кровь в выделениях", "отказ от воды дольше суток"]  // не больше 5
}
```
...
`entries` и `red_flags` из тела ответа вырезаются.
...
Разметка допустима только в `response`. Из `name`, `text`, `note`, `food` и
`red_flags` она снимается на сервере: клиент рисует эти поля обычным
текстом, и `приём **Пульмикорта**` пользователь увидел бы вместе со
звёздочками.

Контекст питомца уезжает модели с оговоркой, что всё перечисленное в нём
**уже заведено** у пользователя. Без неё модель предлагает напоминания на
каждый активный препарат из карточки — в каждом ответе, независимо от
вопроса.
...
| `PROMPT_VERSION`       | `p3`                          | Входит в ключ кэша и в тело ответа                  |
...
клиент уже отправляет её в `pushChat`, и без колонки `urgency` и `red_flags`
не переживут восстановление истории на новом устройстве. Колонка добавляется
руками в админке PocketBase.
