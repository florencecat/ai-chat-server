...
// petContextBlock вкладывает карточку питомца в промт.
//
// Оговорка про «уже заведено» не косметическая. В контексте перечислены
// активные препараты и обработки, и без неё модель предлагает напоминания на
// каждый из них — в каждом ответе, независимо от вопроса: она видит лекарство
// и услужливо «помогает» о нём не забыть. Пользователь получает дубли того,
// что у него уже стоит в календаре.
func petContextBlock(petContext string) string {
	...
	return "Контекст питомца — данные из приложения, считай их достоверными.\n" +
		"ВСЁ перечисленное здесь уже заведено у пользователя: препараты назначены, " +
		"напоминания о них созданы, записи сделаны. Это справочная информация для " +
		"ответа, а НЕ список дел. Не предлагай в events и entries ничего, что уже " +
		"есть в этом блоке, — пользователь увидит дубль того, что у него и так стоит " +
		"в календаре.\n\n" + petContext
}
